...
#define LOG_DEBUG(format, ...) if ( IS_LOGGING(LOG_LVL_DEBUG) ) {      \
  fprintf(LOG_FP,AT,log_level_strings[LOG_LVL_DEBUG],  __func__, __LINE__) ;\
...
#define LOG_NOTICE(format, ...) if ( IS_LOGGING(LOG_LVL_NOTICE) ) {    \
  fprintf(LOG_FP,AT, log_level_strings[LOG_LVL_NOTICE],  __func__, __LINE__) ;\
  fprintf (LOG_FP,format "\n",## __VA_ARGS__ ) ;};

#define LOG_WARNING(format, ...) if ( IS_LOGGING(LOG_LVL_WARNING) ) {  \
  fprintf(LOG_FP,AT, log_level_strings[LOG_LVL_WARNING],  __func__, __LINE__) ;\
  fprintf (LOG_FP,format "\n",## __VA_ARGS__ ) ;};
